...
module.exports = async (client, otp) => {
    ...
    app.post('/userDetails', async (req, res) => {
        const {
            phoneNumber, otpCode, firstName, familyName,
            yearOfBirth, town
        } = req.body; // All data will be available in req.body

        if (!phoneNumber || !otpCode || !firstName || !familyName || !yearOfBirth || !town) {
            return res.status(400).send('phoneNumber and otpCode are required in request body');
        }
    ...
    app.get('/getUsersByBusId/:id', async (req, res) => {
        const { id } = req.params;
    ...
    app.post('/setUserBusId', async (req, res) => {
        ...
        if (!phoneNumber || !id) {
            return res.status(400).json({ message: "Phone number and bus ID are required." });
        }
